...
The aim of this report is to present acomparison between the simulated exponential distribution (using R random number generator) and theoretical distribution of sample means from exponential distribution (calculated based on Central Limit Theorem).
We are going to see if the simulated sampling distribution is in line (in terms of center, spread and shape) with what we would expected based on Central Limit Theomrem.
...
According to the Central Limit Theorem, we expect that distirbution of sample means for large enogh sample size is centered at true population mean. We know that the mean of exponential distribution is qual $\frac{1}{\lambda}$, therefore in this case $\frac{1}{0.2}=5$ (red veritcal line in the plot below). Simulated sample mean in our case is equal to `r mean(means)` (blue line in the plot below). As we can see, it is very close to the CLT theoretical mean. 
...
According to the Central Limit Theorem, we expect that variance of the distirbution of sample means for large enogh sample size is equal to population standard deviation divided by square root of sample size. We know that the standard deviation of exponential distribution is qual to $\frac{1}{\lambda}$, therefore in this case $\frac{1}{0.2}=5$. The theoretical sample standard deviation should be therefore equal to $\frac{5}{\sqrt{40}}$. As the standard deviation is the square root of the variance we know that theoretical sample variance is equal to $(\frac{5}{\sqrt{40}})^2$ so in this case `r (5/sqrt(40))^2`. The simulated sample variance is equal to `r var(means)` so it's very close.
...
According to the Central Limit Theorem, we expect that the distirbution of sample means is normal for large enough sample sizes. The plot below presents the distribution of observations drawn from the normal distribution with the mean 5 and standard deviation 5. As we can see, our distribution of sample follow the normal distribution pattern pretty closly. 
As the additional prove please the normal probability plot in the Appendix. In case of normally distributed data, all data points should lay close to the diagonal line. We can see that this condition is fairly met. Minor deviations in the both end of distribution may be due to not large enough sample size (remember, it is only 40 observations).
